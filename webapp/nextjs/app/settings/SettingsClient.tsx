...
export default function SettingsClient({ initialCompany }: Props) {
  const queryClient = useQueryClient();
  const { data: company, isPending: isLoading } = useQuery(['company', 1], {
    queryFn: async () => initialCompany,
  ...
  useEffect(() => {
    if (company) {
      reset({
        ...
        businesses: company.businesses?.map((b: any) => ({ value: b.description })) || [],
      });
    }
  }, [company, reset]);
  ...
  const onSubmit = (data: CompanyFormValues) => {
    saveMutation.mutate({ ...data, businesses: data.businesses.map((b) => b.value) }, {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries(['company', 1]);
      },
    });
  };
